...
for f in update_xray_config.py server.py index.html; do
    if [ ! -f "$SCRIPTS_DIR/$f" ] && [ -f "/opt/xrayclient/scripts/$f" ]; then
        cp "/opt/xrayclient/scripts/$f" "$SCRIPTS_DIR/$f"
...
if [ ! -f "$CONFIG_DIR/users.txt" ]; then
    if [ -f "/opt/xrayclient/config/users.txt" ]; then
        cp "/opt/xrayclient/config/users.txt" "$CONFIG_DIR/users.txt"
        echo "Copied users.txt to config directory"
    else
